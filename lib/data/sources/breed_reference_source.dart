...
/// Reads the breed traits bundled with the application.
///
/// The live API stopped returning the 1-to-5 ratings and the binary traits on
/// the free plan, so `assets/breed_reference.json` keeps a captured dataset of
/// those fields. It is read once per application run, and the loader is
/// injectable so parsing is testable without Flutter bindings.
class BreedReferenceSource {
  ...
  /// Traits per breed ID, read once per run. Entries that are not objects are
  /// skipped, so a malformed entry cannot drop the whole dataset.
